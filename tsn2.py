...
class getTaggedDocuments(object):
    ...
    def __iter__(self):
        ...
        #for line in open(os.path.join(self.dirname, fname), encoding='cp1252'):    # with different encoding
            #yield LabeledSentence(words=line.split(), tags=['SENT_%s' % count])
            yield TaggedDocument(words=line.split(), tags=['WORD_' + str(count)])
            count += 1
...
def tsne(model):
    ...
    for doc in model.docvecs:
        print(doc)
        doc_2d.append(TSNE().fit_transform(doc).reshape(-1, 1))
...
if __name__ == '__main__':
    ...
    docs = getTaggedDocuments(open(sys.argv[1], encoding='utf-8').readlines())
    

    model = Doc2Vec(size=300, min_count=0, alpha=0.025, min_alpha=0.025)           # TODO: parameter testen
    ...
    # raw output
    np.savetxt('C:\\Users\\mrzl\\phaedra\\doc2vec_reduced.csv', doc_2d, delimiter='\t')
